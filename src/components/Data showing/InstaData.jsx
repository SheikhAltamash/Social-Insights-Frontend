...
export const InstaData = () => {
  ...
  let { case_no, name } = location.state || 1234;
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === "progress") {
        setProgress(data.status); // Update progress in real-time
      }
    };
  ...
  //           try {
  //             const data = JSON.parse(event.data);
  //             if (data && data.pdfPaths) {
  //               setPdfList(data.pdfPaths);
  ...
  //          if(res.ok) {
  //             alert("PDF generation started, wait for the results")
  //          } else {
      ...
      {!loading == true && (
        <div className="div_name_posts">
          ...
          <p className="case_no_post_fb">
            Case number : <span>{data.case_no || 20232}</span>
          </p>
        </div>
      )}
            ...
            <div className="main_post_div">
              {loading ? (
                ...
                <div>
                                          ...
                                          <div className="progress">{progress}Analysing Chats and Posts ...</div>
                </div>
              )}
            </div>
          </div>
